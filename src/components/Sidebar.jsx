export default function Sidebar({onAdd, projects, onSelect}) {
  return (
    <nav className="w-1/3 bg-black max-w-80 p-10 rounded-tr-2xl">
      ...
      <ul className="text-white py-8 text-xl">
        {projects.map(proj => <li key={proj.id}>
            <button onClick={onSelect} className="w-full hover:bg-stone-800 hover:text-stone-50 text-stone-300 rounded-md py-2 px-3 text-left">
              {proj.title}
            </button>
          </li>
        )}
